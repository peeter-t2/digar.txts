...
get_concordances <- function(searchterm,texts,before=30,after=30,txtcolumn="txt",idcolumn="id"){
  ...
  found_index <- texts[, .(locs=unlist(stringr::str_locate_all(txt,searchterm))),by=id][order(id,locs)]#[order(locs),.SD,by=eval(idcolumn)]
  found_index[,var:=rep(c("begin","end"),nrow(found_index)/2)]
