...
get_subset_meta <- function(subset){
  ...
  system(paste0('printf "setupmeta \t $USER \t got corpus meta ', length(metafilelist) ,' files: ', nissues, ' issues from ', minyear, ' to ', maxyear, ' in ', mainpaper, ' (', mainpaper_issues, ')', ' and ', papers, ' other papers' ,' \t" >> /gpfs/hpc/projects/digar_txt/appendOnly_testDir/log1.txt; date +"%Y-%m-%d %T" >> /gpfs/hpc/projects/digar_txt/logs/log1.txt'))
  return(subset_meta)
}
