...
do_subset_search <- function(searchterm = "oskar kallas",searchfile = "oskarkallas.txt",subset){
  ...
  for (seq in 0:floor(length(filelist)/1000)){
    ...
    system(paste0('printf "search \t $USER \t do search ', searchterm, ' in ' , length(filelist),' files: ', nissues, ' issues from ', minyear, ' to ', maxyear, ' in ', mainpaper, ' (', mainpaper_issues, ')', ' and ', papers, ' other papers' ,' \t" >> /gpfs/hpc/projects/digar_txt/logs/log1.txt; date +"%Y-%m-%d %T" >> /gpfs/hpc/projects/digar_txt/logs/log1.txt'))
